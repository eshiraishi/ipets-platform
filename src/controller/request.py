...
async def show_request(
    ...
    document = await commons["db"]["requests"].find_one({"_id": document_id})
    if document is not None:
        return document

    raise HTTPException(
        status_code=404, detail=f"request {document_id} not found"
    )
...
@router.delete("/{document_id}", response_description="Delete a request")
async def delete_request(
    document_id: PyObjectId,
    commons: Annotated[dict, Depends(common_parameters)],
):
    delete_result = await commons["db"]["requests"].delete_one(
        {"_id": document_id}
    )
